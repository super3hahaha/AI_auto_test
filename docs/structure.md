...
│   ├── flow_result_settle.sh # 固化脚本 source 用的 settle_result_page(mode, value)：点保存/转换/合并类操作后，waitfor 判定点命中和紧跟的 ui --field dump 结果页字段之间有窗口期会被"事后插屏广告"二次盖住、读成 <NOTFOUND> 误判失败，dump 前用它按同一个判定点再确认最多3轮（不在就sweep清一轮）。2026-09-04 抽出，17个 flow_*.sh 复用，原理见 gotchas.md 同日条目
