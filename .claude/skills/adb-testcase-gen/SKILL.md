...
面向本项目（AI_auto_test 框架仓库根目录）的执行大脑。目标是把模糊的一句话，
...
2. **预期必须可观察、可判定。** 落在黑盒可见信号上：界面出现的具体文案/控件、输出目录新增的文件、有无崩溃。
   ❌"功能正常" ✅"结果页出现文件 xxx.ext，字段/状态为 yyy"（换成实际输出的例子）。当前被测包多为非 debug，**读不了 DB/SP**，别把预期寄托在数据库字段上。
3. **写清前置态。** 全新启动 vs 已有数据，是不同路径。
4. **凡有音频产物生成的用例，都要带重命名步骤。** 裁剪/合成/转码等一旦生成新音频文件，步骤里必须有"结果页重命名"这一环——这不只是改 UI 文案，还要验证 MediaStore `_data` 路径同步变更，是这类用例的核心校验点。命名规则：`功能简称+YYYYMMDD_HHMMSS`（**精确到秒**，别只到分——同一分钟内连跑两次会因同名导致重命名失败，真机踩过坑，参考 CUT-CORE-01）。功能简称对应当前操作，如裁剪 `cut`、合成 `mix`、转码 `conv`。
5. **`purpose`（测试目的）必须由这条用例自己的 steps/goal 现场提炼，不能从同模块老用例直接抄。** 同模块不同用例测的常常是不同动作（如 `DL-TT-01` 测"下载音频"本身，`DL-TT-02` 测"点 Output Folder 跳转 tab"），`purpose` 抄旧用例会让执行台卡片列表里一堆不同用例显示同一句话、看不出区别。写之前反问自己："这条用例的 steps 实际点了什么、验证了什么？"用那个动作提炼成 4-8 字短语（如"Output Folder跳转"、"重命名链路"），而不是照搬模块里第一条用例的 purpose。留空也比抄错的强（留空默认取 module）。
