...
## 2026-09-09/10：录制器控件框「整体放大」——scrcpy 的虚拟显示屏截胡了 `dumpsys` 里的设备尺寸（已修）

**现象**：三星 A05s（Android 15，1080×2400）上录制器**视频模式**控件框整体比画面大（1.25× 或 1.5×，
顶部贴合、越往右下偏越多）；**静图模式完全正常**；Note9（Android 10）两种模式都正常。用户原话："刚进去
是正常的，闪一下就不对了"——静图先到、视频起流后一闪切过去，基准就换错了。横幅上抓到现行：静图
`基准 1080×2400`，视频 **`基准 864×1920`**。

**根因链（三段都真机实锤）**：
1. 这台机器的 MediaCodec 不吃 1080×2400，scrcpy 报 `Capture/encoding error → Applying video encoder
   constraints` 后沿 **2560→1920→1600→…** 阶梯自动降档：独立复刻取流握手读 session meta + 解 SPS，实测
   编码 **864×1920**（2400/1920 = 1.25；连错两次降到 720×1600 = 1.5，两档正对应两张截图的倍数）。
2. Android 14+ 上 scrcpy 降档镜像会**新建一块虚拟显示屏**，尺寸就是视频尺寸。它出现在
   `dumpsys window displays` 里，而且**排在真机屏前面**：`Display: mDisplayId=17 … cur=864x1920`，
   然后才是 `Display: mDisplayId=0 … cur=1080x2400`（scrcpy 未运行时只有 id=0）。
3. `recorder_daemon.py _device_size_sync()` 全文取**第一个** `\bcur=`——被虚拟屏截胡，返回 864×1920；
   `videoMeta.device` 带着它下发，前端 `base` 用它做百分比分母，bounds 却是 1080×2400 坐标系 → 框整体
   ×1.25。**这个函数只在 scrcpy 起流后才被调用**，所以在 shell 里/脚本里怎么测都对（8/8、3/3 全是
   1080×2400），daemon 里每次都错——"我这边复现不了"的典型：复现条件是"scrcpy 正在跑"。

**修法**：
- `_device_size_sync()` 只在 `Display: mDisplayId=0` 那一块里找 `cur=`（切块再匹配）。
- `videoMeta` **绝不**再用视频尺寸 `(vw, vh)` 兜底——那就是把错误值合法化。取不到时退用最近一次
  screencap 的 PNG 真实像素（与 bounds 同坐标系，daemon 记在 `last_shot_wh`），再没有沿用上次；仍
  为空就发 `device: null`，前端 `deviceWH` 置空、`base` 退回截图基准，宁可暂时不画框也不按错基准画。
- 视频尺寸 ≠ 设备尺寸时 daemon 打 `[scrcpy] 视频 WxH / 设备 WxH（编码器降尺寸）`，一眼可见。
- `videoPipe.ts` canvas 像素尺寸改为跟随帧、帧 1:1 画（去掉"视频≈设备尺寸只差 8 对齐"这条假设；
  框按百分比定位，与 canvas 像素尺寸无关，scrcpy 降档保持宽高比）。这一条不是本次的因，但同一假设
  的另一处依赖，一起拆掉。

**排查中被证伪的（都像，都不是；记下免得重走）**：①前端新树配旧图（已改按 seq 原子配对，真缺陷但
静图模式用户没看到问题）；②WKWebView 重绘/hover（加了 `onImgLoad` 透明度扰动，无害保留）；③u2→shell
后端卡死切换、多 window 坐标基准（这台 shell 后端本就稳定失败切不过去，且业务控件 bounds 两后端一致）；
④孤儿 daemon（确有一个，已清，与本 bug 无关）；⑤"WKWebView drawImage 不按 w/h 拉伸"（我的错误推断，
真因是基准，不是绘制）。**验证过没问题、下次别再查的三层**：同一静止屏连续 u2 dump bounds 零漂移；
`screencap` PNG = `dumpsys` id=0 `cur=` = 树包围盒 = 1080×2400（edge-to-edge，`app=` 亦等于全屏）；
串行采一对 dump+截图离线渲染严丝合缝。

**⚠️ 对齐自检横幅检测不出这个 bug**：它拿同一个 `base` 反算，自洽——基准错了它照样"✓ 已对齐"。已补
`基准 W×H · seq N`，这次就是靠这几个字定位的：**要显示的是"用了哪个数"，而不是"这个数自洽不自洽"**。

**判据**：框整体等比放大（非平移）、只在视频模式、倍数是 2400÷{1920,1600,1280} 之类 → 看横幅 `基准`
是否等于设备尺寸；再看 daemon `[scrcpy] 视频 … / 设备 …`。**教训**：① 解析 `dumpsys` 这类多段输出，
永远先切到目标块再匹配，"第一个匹配"在多显示屏/多窗口下就是随机数；② "取不到就用另一个差不多的值"
是最危险的兜底——差不多的值会让错误静默、且只在别的机器上炸；③ 复现不了时先问"那段代码运行时的
环境和我测试时差在哪"（这里是 scrcpy 在不在跑），而不是继续在能复现的层里加固。

## `t()` 调用时原文里带空格（如「A 到 Z」）不加引号，会被 shell 拆词只取第一个词（2026-09-10，`SEL-SORT-01` 固化）

`tools/lang_helper.sh` 的 `t(){ local text="$1"; ... }` 本身没问题，但调用点如果写
`$AK waitfor text "$(t A 到 Z)" --timeout 8`（`t A 到 Z` 没加引号），shell 在命令替换前就会按空白
把 `A 到 Z` 拆成三个位置参数传给 `t()`——`t()` 只用 `$1`（"A"），后面的"到 Z"被当成从未使用的 `$2`
(`key`) 和一个多余的第三参数丢弃。等价于把 `waitfor text 'A'` 发出去，真机上找不到单独文本为 `A`
的节点，报"等待 8.0s 仍未出现"，看起来像 UI 变了或选择器错了，实际是脚本自己的引号漏了。
**判据**：`t()` 调用的原文本身含 ASCII/全角空格，`waitfor`/`taptext` 报超时，但同一屏截图/dump 里
明明能看到那段完整文案。**改法**：原文含空格必须整体加引号：`$(t "A 到 Z")`；不含空格的原文
（如`选择音频`、`排序方式：`）没有这个问题，两种写法效果一样，容易掉以轻心。**排查现有脚本**：
`grep -rn '\$(t [^"]*[[:space:]][^")]*)' apps/*/flows/*.sh` 找不带引号且含空格的 `t()` 调用点。

## 「选择音频」列表排序弹窗内部选中态不进无障碍树，但列表页头部另有一个持久只读标签能读出当前排序分类（2026-09-10，`SEL-SORT-01` 固化）

固化"排序后记不记得住"这条用例时，先假设能从排序弹窗内部判断当前选中的是「A 到 Z」还是「Z 到 A」——
真机 dump 两个选项节点（`tv_asc`/`tv_desc`）`selected`/`checked` 属性恒为 `false`，高亮纯靠背景色，
无障碍树完全不暴露，没法当断言用。改查列表页本身（不用打开弹窗）：排序入口那一小行（`ll_sort_item`）
除了触发弹窗的 `iv_arrow` 图标，旁边还有一个静态文本 `tv_sort_name`，实时显示当前生效的排序分类
（"名称"/"添加日期"/"长度"/"大小"），弹窗打开与否都在。这个字段比"重新打开弹窗看二级选项"更直接、
更不依赖弹窗内部渲染状态，适合当"排序方式有没有被记住"的主断言（`iv_arrow`/弹窗内部状态只适合当旁证）。
另外该 App 对文件名用的是"自然数字排序"而非逐字节 ASCII 比较（`13h.mp3` 排在 `9` 开头文件之后而不是
`2` 开头文件之前，说明数字前缀按数值整体比较），中文标点/方括号开头的标题在中文语言环境下排序权重
很低、稳定排最前——这两条真机验证过的规则，具体验证方法与断言取舍见 `apps/MP3Cutter/cases/SEL-SORT-01.yaml`
notes。

## 单标签正则按「先 resource-id 后 text」的属性顺序写，会因为真机 dump 里 text 排在 resource-id 前面而静默匹配不到（2026-09-11，`SEL-SEARCH-01` 固化）

固化「文件夹搜索」用例时，为了数进入子文件夹后的文件行数，写了一个正则一次性抠出 `tv_name` 节点的
`text`：`<node[^>]*resource-id="[^"]*id/tv_name"[^>]*text="[^"]*"[^>]*/>`——要求先出现 `resource-id=`
再出现 `text=`。真机跑出来文件行数恒为 0，配的截图却明明看到 3 个文件都在，一度怀疑是纪律#16 那种
"结果页被广告/异步加载盖住、dump 早了"的时序问题，加了 5 轮重试+间隔 1s 仍然 0——说明不是时序，是
正则本身的问题。真机 dump 出来的属性顺序其实是 **`text` 在前、`resource-id` 在后**
（`<node index="2" text="27h.mp3" resource-id=".../id/tv_name" .../>`），正则里
`resource-id="..."[^>]*text="..."` 这段要求 `text=` 出现在 `resource-id=` **之后**，而这条属性顺序
反过来了，`[^>]*` 在标签内怎么滑动都凑不出这个顺序，整条正则对每一个 `tv_name` 节点都不匹配，
`grep -oE` 静默返回空——没有报错、没有警告，看起来跟"这一步压根没找到任何节点"一模一样。
**判据**：抠某个 resource-id 节点的 `text` 恒为空/0，同一份 dump 里能看到该文本明明存在，且不是
纪律#16 说的"异步加载/广告盖屏"那种时序问题（多轮重试间隔真实等待后依然为 0，说明问题不在"晚不晚"
而在"抠不抠得到"）。**改法**：别在一条正则里假设属性顺序，分两步抠——第一步只用
`resource-id="..."` 定位、拿到完整的单个自闭合标签（`<node[^>]*resource-id="[^"]*id/xxx"[^>]*/>`，
不掺 `text=` 条件），第二步再对这个已经框定范围的标签文本单独 `grep -oE 'text="[^"]*"'` 取值——
两步都不关心属性出现的先后顺序，天然免疫这个坑。`extract_folder_rows`（同一脚本里，抠 `name`/`count`
两个字段）一开始就是这么写的，从未踩坑；`extract_tv_names` 图省事写成一步到位的单正则，才踩了这条。
**排查现有脚本**：`grep -rln 'resource-id="\[^"\]\*id/[a-zA-Z_]*"\[^>\]\*text="\[^"\]\*"' apps/*/flows/*.sh`
（或人工检查任何"同一条正则里 `resource-id=` 和 `text=` 两个条件都在、且顺序写死"的写法），命中的
按上面两步拆分法改掉。

## 长按/点击弹出的 PopupMenu 有入场动画，`tapid`紧跟着立刻 `taptext` 偶发会点飞出菜单外（2026-09-11，`SEL-MORE-01` 固化）

固化「文件列表 more 按钮四个菜单项」用例时，`$AK tapid btn_more --index 0` 打开三点菜单后，
紧跟着 `$AK taptext "设置为"` 有时会让整个「更多→设置为」流程直接失败——`taptext` 报告"点了"，
但下一步该出现的 3 选项弹窗完全没出现，画面回到了干净的文件列表，好像菜单点开后自己关掉了。
**用同样的两条命令手动分两次调用（每次调用之间有真实的工具往返间隔）反而每次都能复现成功**，
这个反差是判断依据：不是选择器错、不是权限问题、也不是「更多」菜单本身的缺陷，而是脚本自己
两条命令挨得太近。**根因**：PopupMenu 弹出有入场动画（哪怕很短，一两百毫秒），`tapid` 触发
弹出后立刻紧跟 `taptext` 重新 dump 算坐标，采到的是动画过渡中某一帧的节点 bounds，坐标算出来
是动画中间态的位置而不是最终定妥的位置，点击就落在了菜单外面，菜单被这次"点在空白处"的操作
当成失焦点掉——现象上完全像"点了没反应"或"这一步的功能坏了"。手动分开调用之所以每次都成功，
是因为两次工具调用之间的真实往返延迟（通常远大于动画时长）顺便把动画等完了，纯属巧合掩盖了
问题，不是手动操作本身有什么不同。最初用一句裸 `sleep 0.3` 垫在 `tapid` 和 `taptext` 之间，
真机验证也确实能压住（跑了两轮都过），但这是个"猜时长"的兜底——之后翻 `flow_out_sync.sh` 的
`open_row_action`（2026-09-09 就因为同类"点了没反应"踩过、验证过的既有约定）才发现项目里已经
有更稳的标准写法，换成了那种写法。**改法（照抄 `open_row_action`）**：不各自现 dump，而是
`waitfor id btn_more --cache X` 先确认菜单已经稳定弹出并把这次 dump 存进缓存槽 X，紧接着
`tapid btn_more --from-cache X` 复用同一份 dump 算点击坐标；点开菜单项那步同理，
`waitfor text "菜单项" --cache Y` 确认稳定出现后 `taptext ... --from-cache Y`。两步"确认出现"
和"算坐标"钉死用同一份已验证稳定的 dump，而不是分别各自重新 dump 一次，天然不会跟入场动画
再撞一次，比裸 `sleep` 更可靠（不用猜多久够、跑得快的设备也不多等）。**排查现有脚本**：
`grep -B1 'taptext.*--timeout' apps/*/flows/*.sh`，看紧邻上一行是不是"刚打开一个带动画的
弹出菜单/弹窗"的 `tapid`（比如某个三点/更多按钮）且没有配对的 `waitfor--cache`/
`taptext--from-cache`，有就照 `open_row_action` 这个模板改；不是所有 `tapid`+`taptext`
紧邻都需要——只有"点开会触发一个新弹出层（菜单/对话框/气泡）"这一类才有入场动画的风险，
跳转到全新 Activity/Fragment 页面的点击不受此影响（页面级转场本身有系统级过渡，`waitfor`
的轮询天然覆盖，不会踩这个坑）。
