...
| `mp3-sample-track.mp3`（或等价替代） | `flows/flow_cut_save.sh` 硬编码依赖这个文件名（见下） | **不可再生 + 有版权，禁止入库**：本机放一个同名真实音频（内容不重要，文件名子串"mp3-sample-track"要保留，否则要同步改脚本里的匹配串）。命名规则：`<格式>-sample-track.<ext>`，以后新增 m4a/aac 等格式测试文件时沿用同一规则，如 `m4a-sample-track.m4a` |
